...
// #define DR    0
#define SNOOZE_GDIR  1
// #define SNOOZE_PSR   2
#define SNOOZE_ICR1  3
#define SNOOZE_ICR2  4
#define SNOOZE_IMR   5
#define SNOOZE_ISR   6
#define SNOOZE_EDGE  7
...
void SnoozeDigital::enableDriver(uint8_t mode) {
    ...
    while (__builtin_popcountll(_pin)) {
        ...
        if (mode == 3) {
            ...
            if ((uint32_t)gpio == (uint32_t)&GPIO6_DR) {
                if ((GPIO1_IMR & 0xFFFF0000) && return_gpio1_16_31_irq == 0) {
                    ...
                    return_priority_gpio1_16_31 = NVIC_GET_PRIORITY(IRQ_GPIO1_16_31);
                    NVIC_SET_PRIORITY(IRQ_GPIO1_16_31, priority);
                    __disable_irq();
                    return_gpio1_16_31_irq = _VectorsRam[IRQ_GPIO1_16_31+16];
                ...
                if ((GPIO1_IMR & 0xFFFF) && return_gpio1_0_15_irq == 0) {
                    ...
                    return_priority_gpio1_0_15 = NVIC_GET_PRIORITY(IRQ_GPIO1_0_15);
                    NVIC_SET_PRIORITY(IRQ_GPIO1_0_15, priority);
                    __disable_irq();
                    return_gpio1_0_15_irq = _VectorsRam[IRQ_GPIO1_0_15+16];
            ...
            else if ((uint32_t)gpio == (uint32_t)&GPIO7_DR) {
                if ((GPIO2_IMR & 0xFFFF0000) && return_gpio2_16_31_irq == 0) {
                    ...
                    return_priority_gpio2_16_31 = NVIC_GET_PRIORITY(IRQ_GPIO2_16_31);
                    NVIC_SET_PRIORITY(IRQ_GPIO2_16_31, priority);
                    __disable_irq();
                    return_gpio2_16_31_irq = _VectorsRam[IRQ_GPIO2_16_31+16];
                ...
                if ((GPIO2_IMR & 0xFFFF) && return_gpio2_0_15_irq == 0) {
                    ...
                    return_priority_gpio2_0_15 = NVIC_GET_PRIORITY(IRQ_GPIO2_0_15);
                    NVIC_SET_PRIORITY(IRQ_GPIO2_0_15, priority);
                    __disable_irq();
                    return_gpio2_0_15_irq = _VectorsRam[IRQ_GPIO2_0_15+16];
            ...
            else if ((uint32_t)gpio == (uint32_t)&GPIO8_DR) {
                if ((GPIO3_IMR & 0xFFFF0000) && return_gpio3_16_31_irq == 0) {
                    ...
                    return_priority_gpio3_16_31 = NVIC_GET_PRIORITY(IRQ_GPIO3_16_31);
                    NVIC_SET_PRIORITY(IRQ_GPIO3_16_31, priority);
                    __disable_irq();
                    return_gpio3_16_31_irq = _VectorsRam[IRQ_GPIO3_16_31+16];
                ...
                if ((GPIO3_IMR & 0xFFFF) && return_gpio3_0_15_irq == 0) {
                    ...
                    return_priority_gpio3_0_15 = NVIC_GET_PRIORITY(IRQ_GPIO3_0_15);
                    NVIC_SET_PRIORITY(IRQ_GPIO3_0_15, priority);
                    __disable_irq();
                    return_gpio3_0_15_irq = _VectorsRam[IRQ_GPIO3_0_15+16];
            ...
            else if ((uint32_t)gpio == (uint32_t)&GPIO9_DR) {
                if ((GPIO4_IMR & 0xFFFF0000) && return_gpio4_16_31_irq == 0) {
                    ...
                    return_priority_gpio4_16_31 = NVIC_GET_PRIORITY(IRQ_GPIO4_16_31);
                    NVIC_SET_PRIORITY(IRQ_GPIO4_16_31, priority);
                    __disable_irq();
                    return_gpio4_16_31_irq = _VectorsRam[IRQ_GPIO4_16_31+16];
                ...
                if ((GPIO4_IMR & 0xFFFF) && return_gpio4_0_15_irq == 0) {
                    ...
                    return_priority_gpio4_0_15 = NVIC_GET_PRIORITY(IRQ_GPIO4_0_15);
                    NVIC_SET_PRIORITY(IRQ_GPIO4_0_15, priority);
                    __disable_irq();
                    return_gpio4_0_15_irq = _VectorsRam[IRQ_GPIO4_0_15+16];
            ...
            return_priority_gpio6789 = NVIC_GET_PRIORITY(IRQ_GPIO6789);
            NVIC_SET_PRIORITY(IRQ_GPIO6789, priority);
            __disable_irq();
            return_gpio6789_irq = _VectorsRam[IRQ_GPIO6789+16];
...
void SnoozeDigital::clearIsrFlags(uint32_t ipsr) {
    ...
    while (__builtin_popcountll(_pin)) {
        ...
        uint32_t status = gpio[SNOOZE_ISR] & gpio[SNOOZE_IMR];
        if (status) {
            gpio[SNOOZE_ISR] = status;
        }
...
void SnoozeDigital::attachDigitalInterrupt(uint8_t pin, int type) {
    ...
    gpio[SNOOZE_IMR] &= ~mask;    // disable interrupt
    *mux = 5;                      // pin is GPIO
    gpio[SNOOZE_GDIR] &= ~mask;    // pin to input mode
    uint32_t index = __builtin_ctz(mask);
    if (type == CHANGE) {
        gpio[SNOOZE_EDGE] |= mask;
    } else {
        gpio[SNOOZE_EDGE] &= ~mask;
        if (index < 15) {
            uint32_t shift = index * 2;
            gpio[SNOOZE_ICR1] = (gpio[SNOOZE_ICR1] & ~(3 << shift)) | (icr << shift);
        } else {
            uint32_t shift = (index - 16) * 2;
            gpio[SNOOZE_ICR2] = (gpio[SNOOZE_ICR2] & ~(3 << shift)) | (icr << shift);
        }
    }
    gpio[SNOOZE_ISR] = mask;  // clear any prior pending interrupt
    gpio[SNOOZE_IMR] |= mask; // enable interrupt
}
...
void SnoozeDigital::detachDigitalInterrupt(uint8_t pin) {
    if (pin >= 33) return;
    volatile uint32_t *gpio = portOutputRegister(pin);
    if (sleep_type == 3) {  // Note: In original code this was checking sleep_type == 2, keeping it the same
        if ((uint32_t)gpio == (uint32_t)&GPIO6_DR) {
            gpio = &GPIO1_DR;
        }
        else if ((uint32_t)gpio == (uint32_t)&GPIO7_DR) {
